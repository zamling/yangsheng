...
from loss.seg_detector_loss import LossBuilder
...
def build(args):
    ...
    # xs: a dict like {layer1: NestTensor [(b,c,h,w),(b,h,w)], layer2:...}

    model = get_mvss(sobel=True, n_input=3, constrain=True)
    model.to(device)
    criterion = LossBuilder(eps=1e-6, mask_scale=4, edge_scale=10, cls_scale=5)
    criterion.to(device)
